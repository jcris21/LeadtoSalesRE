...
| G13 | **El historial conversacional del responder vive en memoria** (checkpoint LangGraph no persistido): un restart del proceso pierde el contexto y el LLM vuelve a saludar y re-preguntar dimensiones ya capturadas (observado tras el deploy del fix G11: re-preguntó tipo de propiedad ya extraído en A2). | 🟡 Medio | El perfil no se afecta (vive en DB) y la extracción sigue correcta, pero la UX conversacional se resetea. Pendiente: checkpointer persistente (p. ej. Postgres) o rehidratar historial desde `archived_messages`. |
...
- [x] El mensaje Top-3 llegó a la UI de Chatwoot. ⚠️ Calidad mejorable: las explicaciones no incluyen dirección/precio/link de cada propiedad, y el signal `type match` sale sin traducir — anotar como mejora del `explanation_generator`.
