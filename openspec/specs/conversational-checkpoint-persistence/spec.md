...
Persist LangGraph conversational checkpoints durably so that conversational context survives process restarts, while bounding the active history window and folding evicted turns into a persisted, deterministic summary that remains accessible via `history()`.
