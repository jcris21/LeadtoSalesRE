...
Hybrid Retrieval (US-303/US-304) is the two-stage candidate selection of the Recommendation pipeline: hard-constraint filtering pushed down to a SQL `WHERE` on `properties`, then semantic ranking of the survivors via the pgvector `<->` cosine-distance operator with an HNSW index on Postgres. An in-memory fallback exists only where the dialect lacks pgvector (the SQLite test harness).
