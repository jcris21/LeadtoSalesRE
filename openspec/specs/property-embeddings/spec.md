...
Property embeddings (US-308) power semantic retrieval: real 1536-dimension vectors generated by OpenAI `text-embedding-3-small` behind the async `EmbeddingModel` seam, with a deterministic hash fallback for keyless environments, stored as `vector(1536)` on Postgres. Embeddings are precalculated by the hash-gated ingestion pipeline — never computed on the request path.
