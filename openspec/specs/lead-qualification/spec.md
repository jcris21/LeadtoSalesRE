...
Domain and application-layer capture of the progressive `BuyerProfile` dimensions (budget, locations,
property_type, timeline, must_haves, financing_type, decision_maker_mode) plus the completeness gate
(QA-14) and `ProfileCompleted` event contract. Extended by `buyer-profile-dimensions-us-208` to add the
financing and decision-maker dimensions, and by `lead-objections-classification-us-209` to add
free-text objection detection and Hot/Warm/Cold lead classification.
