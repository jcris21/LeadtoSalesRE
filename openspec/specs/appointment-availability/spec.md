...
Appointment availability (US-402) is the deterministic gate that mitigates the single critical business
risk of the customer journey: booking a visit to a property that is not actually available. It provides
`AvailabilityValidatorPort.check(property_id, slot) -> confirmed | pending | unavailable` as a no-LLM,
deterministic service, backed by an organization-scoped `Broker` entity and an auditable
`AvailabilityCheck` history — including automatic re-validation 2-4h before a booked visit. No downstream
scheduling work (Google Calendar Adapter, Scheduling Service, Reminder Scheduler) may proceed without this
gate.
